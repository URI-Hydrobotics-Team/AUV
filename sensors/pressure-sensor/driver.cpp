...
bool MS5837::init() {
	if(!bcm2835_init()){
		std::cout << "Failed to init bcm2835\n";
		return false;
	}


	if(!bcm2835_i2c_begin()){
		std::cout << "Failed to start i2c interface\n";
		return false;
	}
	...
	for ( uint8_t i = 0 ; i < 7 ; i++ ) {
		...
		uint8_t ret_val;
		//get 2 bytes
		bcm2835_i2c_write_read_rs(sendBuf,1,recvBuf,2);
...
void MS5837::read() {
	...
	bcm2835_i2c_write_read_rs(sendBuf,1,recvBuf,3);
